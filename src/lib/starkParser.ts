...
export type SeverityLabel = 'Critical' | 'Serious' | 'Moderate' | 'Minor' | 'Unknown';
...
export function formatSeverityLabel(severity: SeverityLabel, scheme: SeverityScheme | undefined): string {
	if (scheme === 'hml') {
...
export function categorizeIssue(input: Pick<ConsolidatedIssue, 'title' | 'description' | 'ruleId' | 'wcag'>): IssueCategory {
	const hay = normalizeText(`${input.ruleId ?? ''} ${input.wcag ?? ''} ${input.title} ${input.description}`).toLowerCase();

	// Order matters: first match wins.
	if (/(^|\b)(alt|alt text|alternate text)\b|missing\s+alt|image\s+.*\balt\b|non-text\s+content/.test(hay)) return 'Alt text';
	if (/contrast|4\.5|3:1|color\s+contrast|background\s+is\s+an\s+image/.test(hay)) return 'Color contrast';
	if (/label\b|labels\b|form\s+field|input\b|textarea\b|select\b|aria-label\b.*(input|textbox)|missing\s+label/.test(hay)) return 'Form labels';
	if (/link\b|links?\s+must\s+have\s+discernible\s+text|empty\s+link|link\s+name|anchor\b/.test(hay)) return 'Link text';
	if (/button\b|buttons?\s+must\s+have\s+discernible\s+text|button\s+name/.test(hay)) return 'Button text';
	if (/heading\b|headings\b|h1\b|h2\b|h3\b|heading\s+level|outline/.test(hay)) return 'Headings';
	if (/aria-(label|labelledby|describedby|hidden)\b|role\b|landmark\b|region\b|navigation\b.*aria/.test(hay)) return 'ARIA / landmarks';
	if (/keyboard\b|focus\b|tab\s+order|visible\s+focus|operable/.test(hay)) return 'Keyboard / focus';
	if (/(\blang\b|language\s+of\s+page|language\s+of\s+parts)/.test(hay)) return 'Language';
	if (/table\b|th\b|scope\b|header\s+cell|data\s+table/.test(hay)) return 'Tables';
	return 'Other';
}
...
function isAltTextRelated(description: string): boolean {
	...
	return (
		/(^|\b)(alt|alt text|alternate text)\b|missing\s+alt|image\s+.*\balt\b|non-text\s+content/.test(hay)
	);
}
...
function isUsefulAltExampleSnippet(snippet: string): boolean {
	...
	return /<\s*img\b/.test(s) || /\balt\s*=/.test(s) || /<\s*picture\b/.test(s) || /<\s*source\b/.test(s);
}
...
function headerLooksLikeIssuesTable(headerKeys: string[]): boolean {
	...
	const hasDescriptionLike = [...set].some((h) => /(description|details|why it matters|explanation)/.test(h));
	const hasTitleLike = [...set].some((h) => /(title|issue|problem|summary|rule|check)/.test(h));
...
function extractBreakdownTotals(doc: Document): { violations?: number; potentialViolations?: number } {
	...
	const findMetric = (label: RegExp): number | undefined => {
		...
		for (let i = 0; i < 6 && cur; i += 1) {
			...
			const inner = Array.from(container.querySelectorAll('div,span')) as Element[];
...
function extractWcagBreakdownTotals(doc: Document): WcagCriterionTotals[] {
	...
	const svgs = Array.from(
		doc.querySelectorAll('svg[aria-label="Failures"], svg[aria-label="Potentials"]')
	);
	...
	for (const svg of svgs) {
		...
		const numberText =
			normalizeText(wrapper?.querySelector('div')?.textContent) || normalizeText(wrapper?.textContent);
...
function parseTableIssues(table: HTMLTableElement, sourceFile: string): ParsedIssue[] {
	const rows = Array.from(table.querySelectorAll('tr'));
	if (rows.length === 0) return [];

	const headerRow =
		table.tHead?.rows?.[0] ??
		(rows.find((r) => r.querySelectorAll('th').length > 0) ?? rows[0]);
	const headerCells = Array.from(headerRow.querySelectorAll('th,td')).map((c) => getCellText(c));
	...
	const occurrencesIdx = colIndex(/(occurrences|occurrence|instances|instance|count|violations|items)/);
	const titleIdx = colIndex(/(title|issue|problem|summary|rule)/);
	const descriptionIdx = colIndex(/(description|details|why it matters|explanation)/);
	...
	const dataRows = bodyRows.filter((r) => r !== headerRow && r.querySelectorAll('td,th').length > 0);
	const issues: ParsedIssue[] = [];

	for (const r of dataRows) {
		const cells = Array.from(r.querySelectorAll('td,th')).map((c) => getCellText(c));
		const get = (idx: number | undefined): string => (idx === undefined ? '' : normalizeText(cells[idx] ?? ''));
...
function parseHeuristicIssues(doc: Document, sourceFile: string): ParsedIssue[] {
	// Fallback heuristic: look for repeated "Severity" labels near issue blocks.
	const issues: ParsedIssue[] = [];
	const candidates = Array.from(doc.querySelectorAll('section, article, li, div'));
	...
	for (const el of candidates) {
		...
		const sevMatch = text.match(/(?:severity|impact)\s*[:\-]?\s*(critical|serious|high|moderate|medium|minor|low)/i);
...
function parseCategoriesCardIssues(doc: Document, sourceFile: string, pageUrl?: string): ParsedIssue[] {
	...
	for (const el of elements) {
		...
		const fromAriaOccurrences = match?.[3] ? Number.parseInt(match[3], 10) : undefined;

		// Prefer visible card title text if present
		let description = normalizeText((el as Element).querySelector('.col-[title]')?.textContent);
		if (!description) description = fromAriaMessage;
		description = description.replace(/\s*[–-]\s*\(\d+\s+instances?\)\s*$/i, '');

		const occurrences =
			(fromAriaOccurrences && Number.isFinite(fromAriaOccurrences) && fromAriaOccurrences > 0
				? fromAriaOccurrences
...
export function parseStarkHtmlReport(html: string, sourceFile: string): { issues: ParsedIssue[]; debug: ParseDebugInfo } {
	...
	if (reportTotals.violations !== undefined || reportTotals.potentialViolations !== undefined) {
	...
	tables.forEach((t, index) => {
		const headerRow = t.tHead?.rows?.[0] ?? t.querySelector('tr:has(th)') ?? t.querySelector('tr');
		const headers = headerRow
			? Array.from(headerRow.querySelectorAll('th,td')).map((c) => normalizeText(c.textContent))
			: [];
		debug.tableHeaders.push({ index, headers, rows: t.querySelectorAll('tr').length });
	...
	if (issues.length === 0) {
		...
		if (categoryIssues.length > 0 || wcagBreakdown.length > 0) {
			if (categoryIssues.length > 0) {
				debug.warnings.push('No issue tables matched; parsed Categories-style issue cards.');
			}
			if (wcagBreakdown.length > 0) {
				debug.warnings.push('WCAG breakdown counts detected; using them to ensure totals align even if sections are collapsed.');
			}
			...
			if (wcagBreakdown.length > 0) {
				const covered = new Map<string, { failures: number; potentials: number }>();
				for (const i of categoryIssues) {
					...
					else if (i.severity === 'Moderate') existing.potentials += i.occurrences;
					covered.set(key, existing);
				}

				const synth: ParsedIssue[] = [];
				for (const c of wcagBreakdown) {
					const existing = covered.get(c.wcag) ?? { failures: 0, potentials: 0 };
					const missingFailures = Math.max(0, c.failures - existing.failures);
					const missingPotentials = Math.max(0, c.potentials - existing.potentials);
...
export function consolidateIssues(rawIssues: ParsedIssue[]): ConsolidatedIssue[] {
	...
	for (const issue of rawIssues) {
		...
		const key = `${ruleId || ''}|${wcag || ''}|${title}|${description}`.toLowerCase();

		const existing = map.get(key);
		if (existing) existing.issues.push({ ...issue, title, description, ruleId: ruleId || undefined, wcag: wcag || undefined });
		else map.set(key, { issues: [{ ...issue, title, description, ruleId: ruleId || undefined, wcag: wcag || undefined }] });
	}

	const consolidated: ConsolidatedIssue[] = [];
	for (const { issues } of map.values()) {
		const pages = Array.from(new Set(issues.map((i) => normalizeText(i.page)).filter(Boolean)));
		const sourceFiles = Array.from(new Set(issues.map((i) => i.sourceFile)));
		const occurrences = issues.reduce((acc, i) => acc + (Number.isFinite(i.occurrences) ? i.occurrences : 1), 0);
		const exampleSnippets = Array.from(
			new Set(
				issues
					.map((i) => clampSnippet(i.exampleSnippet ?? ''))
					.filter(Boolean)
			)
		).slice(0, 8);
		consolidated.push({
...
export function consolidatedIssuesToHtmlDigest(issues: ConsolidatedIssue[], options: HtmlDigestOptions = {}): string {
	...
	for (const issue of filteredIssues) totalsBySeverity[issue.severity] += issue.occurrences;

	const groups: Array<{ severity: SeverityLabel; issues: ConsolidatedIssue[] }> = [
	...
	const sevLabel = (s: SeverityLabel) => formatSeverityLabel(s, options.severityScheme);
	...
	const issueCard = (i: ConsolidatedIssue) => {
		const pagesDetails = i.pages.length
			? `<details class="sources"><summary>Source pages (${i.pages.length})</summary><div class="sourceList">${i.pages
					...
					.join('')}</div></details>`
			: '';
		const wcag = i.wcag ? `<span class="pill">WCAG ${escapeHtml(i.wcag)}</span>` : '';
		const rule = i.ruleId ? `<span class="pill">Rule ${escapeHtml(i.ruleId)}</span>` : '';
	...
	const totalOccurrences = filteredIssues.reduce((acc, i) => acc + i.occurrences, 0);
	const generatedLabel = generatedAt.toISOString().replace('T', ' ').slice(0, 19);
	...
	<div class="container">
		...
		${groups
			.map((g) => {
				...
				const severityTotal = g.issues.reduce((acc, i) => acc + i.occurrences, 0);
