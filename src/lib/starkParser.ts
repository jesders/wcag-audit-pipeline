...
export function consolidatedIssuesToHtmlDigest(
  ...
  const tabsSeverities: SeverityLabel[] = [
    "Critical",
    "Serious",
    "Moderate",
    "Minor",
...
<body>
    ...
    <div class="footer">Generated ${escapeHtml(generatedLabel)} • ${totalUnique} unique issues • ${totalOccurrences} total occurrences • Generated by WCAG Audit Pipeline (local, in-browser parsing).</div>
  </div>
  <script>
    (() => {
      ...
      function applyFilters() {
        ...
        for (const p of panels) {
          ...
          const pItems = Array.from(p.querySelectorAll('[data-owner-item]'));
          for (const el of pItems) {
