...
export function estimateForCategory(category: IssueCategory, occurrences: number, severity: SeverityLabel): Estimate {
	...
	if (category === 'Color contrast') {
		assumptions.push('Assumes a small number of shared design tokens or CSS variables can fix multiple failures.');
		assumptions.push('If contrast failures are baked into images, effort increases (asset updates).');
	}
	if (category === 'Alt text') assumptions.push('Assumes images are content-managed or have accessible text equivalents available.');
	if (category === 'Form labels') assumptions.push('Assumes form fields can be labelled with <label> or aria-label/aria-labelledby without redesign.');
	if (category === 'Keyboard / focus') assumptions.push('Assumes interactive elements can be made keyboard reachable and focus styles are allowed by design.');
...
export function computeIssueKey(issue: Pick<ConsolidatedIssue, 'ruleId' | 'wcag' | 'title' | 'description'>): string {
...
export function estimateForConsolidatedIssue(issue: ConsolidatedIssue): Estimate {
	const category = categorizeIssue(issue);
	return estimateForCategory(category, issue.occurrences, issue.severity);
}

function recommendationForCategory(category: IssueCategory): { summary: string; steps: string[]; verification: string[] } {
	switch (category) {
		case 'Color contrast':
			return {
				summary: 'Adjust foreground/background colors to meet contrast ratios (4.5:1 normal text, 3:1 large text and UI components).',
				steps: [
		...
		case 'Alt text':
			return {
				summary: 'Add meaningful alt text for informative images; use empty alt (alt="") for decorative images.',
				steps: [
		...
		case 'Form labels':
			return {
				summary: 'Ensure every form control has an accessible name via <label>, aria-label, or aria-labelledby.',
				steps: [
		...
		case 'Link text':
			return {
				summary: 'Make link text descriptive (avoid “click here”); ensure icon-only links have accessible names.',
				steps: [
					'Update anchor text to describe destination or action.',
					'For icon-only links, add visible text, sr-only text, or aria-label.'
				],
				verification: ['Re-run Stark and confirm “discernible link text” issues are resolved.']
			};
		case 'Button text':
			return {
		...
		case 'Headings':
			return {
				summary: 'Fix heading structure (H1..H6) to reflect page outline and avoid skipping levels.',
				steps: ['Ensure each page has a single H1 that describes the page.', 'Use headings in order; do not skip levels for styling.'],
				verification: ['Use a screen reader rotor/headings list; verify the outline is logical.']
			};
		case 'ARIA / landmarks':
			return {
				summary: 'Use native semantics first; add ARIA only when necessary and ensure roles/labels are valid.',
				steps: [
				...
				verification: ['Re-run Stark; spot-check landmarks with a screen reader rotor.']
			};
		case 'Keyboard / focus':
			return {
				summary: 'Ensure all interactive elements are keyboard operable and focus is visible.',
				steps: [
				...
				verification: ['Do a full keyboard pass of key flows; re-run Stark and confirm keyboard/focus issues improve.']
			};
		case 'Language':
			return {
				summary: 'Set the page language (lang) and mark language changes within content where needed.',
				steps: ['Ensure <html lang="..."> is present and correct.', 'For mixed-language content, use lang on specific elements.'],
				verification: ['Re-run Stark and confirm language-related checks pass.']
			};
		case 'Tables':
			return {
				summary: 'Ensure data tables use proper headers (<th>, scope) and associations.',
				steps: ['Add <th> for header cells and scope attributes when appropriate.', 'Avoid tables for layout.'],
				verification: ['Navigate the table with a screen reader; confirm header associations are announced.']
			};
		default:
			return {
				summary: 'Address the underlying issue following WCAG guidance and best practices.',
				steps: ['Identify the component/content source causing the issue and fix at the source.', 'Re-test after changes.'],
...
function groupIssues(issues: ConsolidatedIssue[]): PlanIssueGroup[] {
	...
	for (const i of issues) {
		...
		const rank: Record<SeverityLabel, number> = { Critical: 4, Serious: 3, Moderate: 2, Minor: 1, Unknown: 0 };
		if (rank[i.severity] > rank[existing.severity]) existing.severity = i.severity;
		for (const p of i.pages ?? []) if (p && !existing.pages.includes(p)) existing.pages.push(p);
		for (const s of i.sourceFiles ?? []) if (s && !existing.sourceFiles.includes(s)) existing.sourceFiles.push(s);
	}
...
export function issuesToRemediationPlanHtml(
	...
	const title = options.reportTitle ?? 'Accessibility Remediation Recommendations';
	...
	const scoped = scopeCategory ? issues.filter((i) => categorizeIssue(i) === scopeCategory) : issues;
	...
	const categories = Array.from(byCategory.entries())
		.map(([category, list]) => {
			...
			const estimatedHours = list.reduce((acc, i) => {
				...
				return acc + (typeof o?.hours === 'number' && Number.isFinite(o.hours) ? o.hours : 0);
			}, 0);
			const estimatedCount = list.reduce((acc, i) => {
				const o = overrides[i.key];
				return acc + (typeof o?.hours === 'number' && Number.isFinite(o.hours) ? 1 : 0);
			}, 0);
			return { category, list, occurrences, estimatedHours, estimatedCount };
		})
		.sort((a, b) => b.occurrences - a.occurrences);

	const totalEstimatedHours = categories.reduce((acc, c) => acc + c.estimatedHours, 0);
	const totalEstimatedCount = categories.reduce((acc, c) => acc + c.estimatedCount, 0);
	...
	const issueRow = (i: PlanIssueGroup) => {
		...
		const meta = [i.wcag ? `WCAG ${i.wcag}` : '', i.ruleId ? `Rule ${i.ruleId}` : ''].filter(Boolean).join(' • ');
		const pages = i.pages.length ? `<details><summary>Pages (${i.pages.length})</summary><ul>${i.pages
		...
		const notes = override?.notes ? `<details><summary>Notes</summary><div class="desc">${escapeHtml(override.notes)}</div></details>` : '';
		return `
			<article class="issue">
				<div class="issueTop">
					<div>
...
<body>
	<div class="container">
		...
		<div class="grid">
			<div class="card"><div class="big">${totalEstimatedCount > 0 ? toHours(totalEstimatedHours) : '—'}</div><div class="small">Your estimate total (${totalEstimatedCount} of ${totalGroups} groups)</div></div>
			<div class="card"><div class="big">${unestimatedCount}</div><div class="small">Unestimated groups</div></div>
			<div class="card"><div class="big">${grouped.reduce((acc, i) => acc + i.occurrences, 0)}</div><div class="small">Total occurrences (grouped)</div></div>
			<div class="card"><div class="big">${categories.length}</div><div class="small">Categories affected</div></div>
		</div>
