...
function formatSeverityBadge(severity: ParsedIssue['severity'], scheme: SeverityScheme) {
			...
			: severity === 'Serious'
				...
				: severity === 'Moderate'
					...
					: severity === 'Minor'
						...
						: 'bg-slate-600';
	return (
		<span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-semibold text-white ${color}`}>
			{label}
		</span>
	);
}
...
export function StarkConsolidator() {
	...
	useEffect(() => {
		try {
			localStorage.setItem('stark-remediation-overrides-v1', JSON.stringify(overrides));
		} catch {
			// ignore
		}
	}, [overrides]);
	...
	async function onFilesSelected(fileList: FileList | null) {
		...
		try {
			...
			for (const file of files) {
				...
				debugByFile.push({ fileName: file.name, issuesFound: parsed.issues.length, debug: parsed.debug });
			}
			...
			const scheme: SeverityScheme = schemes.includes('hml') ? 'hml' : schemes.includes('axe') ? 'axe' : 'unknown';
	...
	function setEstimate(issue: ConsolidatedIssue, hours: number | undefined) {
		...
		setOverrides((prev) => {
			...
			if (hours === undefined) {
				...
				if (!next[key]?.notes && next[key]?.hours === undefined) delete next[key];
				return next;
			}
		...
		<div className="mx-auto max-w-6xl p-6">
			<div className="rounded-2xl border border-slate-200/70 bg-white/80 p-6 shadow-sm backdrop-blur dark:border-white/10 dark:bg-slate-900/40">
				<div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
					<div>
						<h1 className="text-xl font-semibold tracking-tight">Stark Remediation Planner</h1>
				...
				{issues.length > 0 && (
					<div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
						...
						<div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4 dark:border-white/10 dark:bg-white/5">
							<div className="text-2xl font-semibold tracking-tight">{totalIssues}</div>
							<div className="mt-1 text-sm text-slate-600 dark:text-slate-300">Total occurrences</div>
						</div>
					{reportedTotals.hasAny && (
							<div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4 dark:border-white/10 dark:bg-white/5">
								<div className="text-2xl font-semibold tracking-tight">{reportedTotals.total}</div>
								<div className="mt-1 text-sm text-slate-600 dark:text-slate-300">Report total (violations + potential)</div>
							</div>
					)}
					{reportedTotals.hasAny && (
							<div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4 dark:border-white/10 dark:bg-white/5">
								<div className="text-2xl font-semibold tracking-tight">{reportedTotals.violations}</div>
								<div className="mt-1 text-sm text-slate-600 dark:text-slate-300">Report violations</div>
							</div>
					)}
					{reportedTotals.hasAny && (
							<div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4 dark:border-white/10 dark:bg-white/5">
								<div className="text-2xl font-semibold tracking-tight">{reportedTotals.potentials}</div>
								<div className="mt-1 text-sm text-slate-600 dark:text-slate-300">Report potential violations</div>
							</div>
					)}
						<div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4 dark:border-white/10 dark:bg-white/5">
							<div className="text-2xl font-semibold tracking-tight">{parsedFiles.length}</div>
							<div className="mt-1 text-sm text-slate-600 dark:text-slate-300">Files parsed</div>
						</div>
					</div>
				)}
					...
					<div className="mt-6 overflow-x-auto rounded-2xl border border-slate-200 dark:border-white/10">
						...
						<thead>
							<tr className="bg-slate-50 text-xs font-semibold uppercase tracking-wide text-slate-600 dark:bg-white/5 dark:text-slate-300">
								...
								<th scope="col" className="px-3 py-3 text-left">Occurrences</th>
								<th scope="col" className="px-3 py-3 text-left">Title</th>
								<th scope="col" className="px-3 py-3 text-left">WCAG</th>
								<th scope="col" className="px-3 py-3 text-left">Pages</th>
								<th scope="col" className="px-3 py-3 text-left">Your est (hrs)</th>
								<th scope="col" className="px-3 py-3 text-left">Notes</th>
							</tr>
						</thead>
							...
							{issues.slice(0, 50).map((i, idx) => {
								...
								return (
									<tr key={`${key}-${idx}`} className="align-top">
										<td className="px-3 py-3">{formatSeverityBadge(i.severity, severityScheme)}</td>
										<td className="px-3 py-3 text-slate-700 dark:text-slate-200">{cat}</td>
										<td className="px-3 py-3 text-slate-700 dark:text-slate-200">{i.occurrences}</td>
										<td className="px-3 py-3 font-medium text-slate-900 dark:text-slate-50">{i.title}</td>
										<td className="px-3 py-3 text-slate-700 dark:text-slate-200">{i.wcag ?? ''}</td>
										<td className="px-3 py-3 text-slate-700 dark:text-slate-200">{i.pages.length}</td>
				...
				{showDebug && parsedFiles.length > 0 && (
					<div className="mt-8">
						<h2 className="text-lg font-semibold tracking-tight">Parse debug</h2>
					{parsedFiles.map((f) => (
							<details
								key={f.fileName}
							...
							<summary>
									...
									<span className="ml-2 text-sm text-slate-600 dark:text-slate-300">
										— {f.issuesFound} issues, {f.debug.matchedTables}/{f.debug.tablesFound} tables matched
									</span>
							</summary>
								<div className="mt-3">
								{f.debug.reportTotals && (
										<p className="text-sm text-slate-600 dark:text-slate-300">
										Report totals — Violations: {f.debug.reportTotals.violations ?? '—'}, Potential: {f.debug.reportTotals.potentialViolations ?? '—'}
									</p>
								)}
								{f.debug.wcagBreakdownTotals && (
										<p className="text-sm text-slate-600 dark:text-slate-300">
										WCAG breakdown — Criteria: {f.debug.wcagBreakdownTotals.criteriaWithCounts}, Failures: {f.debug.wcagBreakdownTotals.failures}, Potentials:{' '}
										{f.debug.wcagBreakdownTotals.potentials}
									</p>
								)}
								{f.debug.warnings.length > 0 && (
									...
									<thead>
												<tr className="bg-slate-50 text-[11px] font-semibold uppercase tracking-wide text-slate-600 dark:bg-white/5 dark:text-slate-300">
													...
													<th scope="col" className="px-3 py-2 text-left">Rows</th>
													<th scope="col" className="px-3 py-2 text-left">First-row headers</th>
												</tr>
									</thead>
											<tbody className="divide-y divide-slate-200 bg-white dark:divide-white/10 dark:bg-transparent">
										{f.debug.tableHeaders.slice(0, 10).map((t) => (
											<tr key={t.index}>
														<td className="px-3 py-2 text-slate-700 dark:text-slate-200">{t.index}</td>
														<td className="px-3 py-2 text-slate-700 dark:text-slate-200">{t.rows}</td>
														<td className="px-3 py-2 text-slate-700 dark:text-slate-200">
															{t.headers.filter(Boolean).slice(0, 12).join(' | ')}
														</td>
											</tr>
										))}
									</tbody>
